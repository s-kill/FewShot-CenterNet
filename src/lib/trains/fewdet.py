...
#TODO Check FewdetLoss class for new heads
class FewdetLoss(torch.nn.Module):
  def __init__(self, opt):
    super(FewdetLoss, self).__init__()
    self.crit = torch.nn.MSELoss() if opt.mse_loss else FocalLoss()
    self.crit_ss = torch.nn.CrossEntropyLoss() #Add loss criterion for ss loss
  ...
  def forward(self, outputs, batch):
    opt = self.opt
    ss_loss, hm_loss, wh_loss, off_loss = 0, 0, 0, 0 #Add ss_loss
    for s in range(opt.num_stacks): #should be 1 in our case
      output = outputs[s]
      if not opt.mse_loss: #opt.mse_loss = False, so sigmoid is applied to 'hm'
      ...
      if opt.wh_weight > 0: #wh_weight = 0.1 so this is True
        ...
        elif opt.cat_spec_wh: #False in our case
      ...
      if opt.reg_offset and opt.off_weight > 0: #True in training
...
class FewdetTrainer(BaseTrainer):
