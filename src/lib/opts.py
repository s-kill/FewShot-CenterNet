...
class opts(object):
  def __init__(self):
    ...
    self.parser.add_argument('--arch', default='dla_34', 
                             help='model architecture. Currently tested'
                                  ...
                                  'dlav0_34 | dla_34| dlav2_34 | dlav3_34 | hourglass')
    ...
    self.parser.add_argument('--ss_weight', type=float, default=1,
  ...
  def update_dataset_info_and_set_heads(self, opt, dataset):
    ...
    elif opt.task == 'ctdet' or opt.task == 'ctdetv2' or opt.task == 'ctdetv3':
      # assert opt.dataset in ['pascal', 'coco']
      opt.heads = {'hm': opt.num_classes,
                   'wh': 2 if not opt.cat_spec_wh else 2 * opt.num_classes}
      if opt.reg_offset:
        opt.heads.update({'reg': 2})
    elif opt.task == 'fewdet' or opt.task == 'fewdetv2' or opt.task == 'fewdetv3':
      # assert opt.dataset in ['pascal', 'coco']
      opt.heads = {'ss': opt.num_classes,
                   'hm': 1,
                   'wh': 2 if not opt.cat_spec_wh else 2 * opt.num_classes}
      if opt.reg_offset:
        opt.heads.update({'reg': 2})
  ...
  def init(self, args=''):
    default_dataset_info = {
      'ctdet': {'default_resolution': [512, 512], 'num_classes': 80, 
                ...
                'dataset': 'coco'},
      'ctdetv2': {'default_resolution': [512, 512], 'num_classes': 80, 
                'mean': [0.408, 0.447, 0.470], 'std': [0.289, 0.274, 0.278],
                'dataset': 'coco'},                
      'fewdet': {'default_resolution': [512, 512], 'num_classes': 80, 
                'mean': [0.408, 0.447, 0.470], 'std': [0.289, 0.274, 0.278],
                'dataset': 'coco'},
